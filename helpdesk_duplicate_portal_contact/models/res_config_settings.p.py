...
class ConfSetting(models.TransientModel):
    ...
    helpdesk_on_duplicate = fields.Boolean(
        string="Duplicate Contacts", store=True, config_parameter='helpdesk.helpdesk_on_duplicate')
    helpdesk_team_id = fields.Many2one(
        comodel_name="helpdesk.ticket.team", string="Helpdesk Team", config_parameter='helpdesk.helpdesk_team_id')
